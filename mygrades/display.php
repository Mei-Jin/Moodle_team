	...
	public function grade_data($report) {
		...
		if ($courses = enrol_get_users_courses($report->user->id, false, 'id, shortname, showgrades')) {
			...
			foreach ($courses as $course) {
                ...
                $courselink,
                 grade_format_gradevalue($finalgrade, $course_item, true),
                ));
                // -- 4th July
			}
     ...
     public function set_table()
        ...
        */  
        ...
        $tablecolumns = array(
            ...
            'grade');
        $tableheaders = array(
            get_string('gradetblheader_course', 'block_mygrades'),
            get_string('gradetblheader_grade', 'block_mygrades'));
        $this->table = new flexible_table('mygrades-' . $USER->id);
